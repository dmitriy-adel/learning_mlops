...
- Также в Go функция может принимать неопределенной кол-во параметров одного типа: 
...
- Также в гошке каждая функция имеет свой тип. Например, `func add(x int, y int) int` имеет тип `func(int, int) int`
