...
- Условная конструкция `if..then` выполняет набор команд, если только выполняется определенное условие. Она имеет следующий синтаксис:
    ...
    После `if` указывается условие, которое представляет команду `test`. После then идет набор команд, которые выполняются, если условие истинно. Заканчивается вся конструкция словом `fi`. Например, определим скрипт hello со следующим кодом:
    ...
    Стоит отметить, что можно сократить условие следующим образом:
...
- Также существует оператор `elif`, логика его, думаю, понятна. Также выше в примерах сравнение чисел производится через оператор `-eq`. В bash нет воможности сравнивать значения при помози привычных операторов ('=', '>', '<'), вместо этого такая логика: 
...
- Определим скрипт hello, который будет иметь следующий код:
    ...
    ```
    Здесь в конструкции `case` проверяется значение переменной *N*. А именно проверяется три варианта. Например, если переменная равна 1, то выполняется команда *echo "num = 1"* и так далее. Также если одно из значений успешно сопоставлено с выражением, то сопоставление с остальными значениями не происходит.

- Иногда бывает нужно обработать все остальные варианты, то есть выполнить некоторый набор команд по умолчанию. В этом случае мы можем использовать шаблон `*`, который соответствует любому значению:
...
- Bash предоставляет следующие типы циклов:
...
- Простейший пример использования цикла `while`, в котором происходит увеличение переменно *N* от 0 до 5 с выводом каждого значения переменной *N*:
...
- Цикл `for` выполняет набор команд определенное количество раз. имеет следующий синтаксис:

    ```bash
    for (( counter; condition; counter_increment)); do
    commands
    done
    ```

    Для примера выведем на экран все числа от 1 до 5 включительно :
    ...
    Для определения условий можно использовать стандартные операторы сравнения: < (меньше чем), > (больше чем), <= (меньше чем или равно), > (больше чем или равно), == (равно), != (не равно). Также можно задавать различные условия инкрементирования счетчика - ниже приведен пример с шагом сразу в 2 единицы: 
    ...
    Если треюуется уменьшать значение счетчика на протяжении цикла, это стоит просто прописать в условии - никаких конструкций нет
...
- Особая форма цикла `for` - `for..in` позволяет перебрать набор значений:
    ...
    Для примера здесь перебираются все строки из файла "/etc/passwd". Для получения строк применяется команда cat /etc/passwd, то есть вывод этой команды фактически представляет набор строк. Каждая строка помещается в переменную t. И для каждой строки выполняется команда echo $t
